...
# ============================================================
# Palette (Neon Arcade theme)
# ============================================================
BG_SCANLINE_1 = (10, 0, 20)   # #0A0014 — near-black cabinet background
BG_SCANLINE_2 = (4, 0, 8)     # faint darker scanline band for a CRT feel

PANEL_BORDER = (0.06, 0.0, 0.12, 1)         # very dark purple — pressed-button feedback
PANEL_BORDER_LIGHT = (0.180, 0.878, 1.0, 1)  # #2EE0FF cyan — default panel/track border
INSET_BG = (0.078, 0.0, 0.169, 1)           # #14002B dark purple — card/row fill
BUTTON_FILL = (0.102, 0.0, 0.2, 1)          # #1A0033 — button/knob/pill body fill
BUTTON_BEVEL_LIGHT = (1.0, 0.180, 0.878, 1)  # #FF2EE0 magenta — button highlight edge

ACCENT = (1.0, 0.180, 0.878, 1)             # #FF2EE0 magenta — primary neon glow color
ACCENT2 = (0.180, 0.878, 1.0, 1)            # #2EE0FF cyan — secondary neon glow color (titles, song text)
ACCENT_DIM = (0.6, 0.35, 0.55, 1)           # muted magenta-purple for secondary labels
SONG_TEXT_MUTED = (0.725, 0.561, 0.839, 1)  # #B98FD6 muted lavender — queue song filenames

CONNECTION_ON_COLOR = (0.180, 0.878, 1.0, 1)   # cyan lightning bolt when connected
CONNECTION_OFF_COLOR = (0.5, 0.45, 0.55, 1)    # muted grey-purple when not connected


def make_scanline_texture(base_color, line_color, tile=4):
    """Small tileable horizontal-band texture for a CRT scanline effect."""
    buf = bytearray()
    for y in range(tile):
        row_color = base_color if y % 2 == 0 else line_color
        for _x in range(tile):
            buf.extend(row_color)
...
class MusicPlayerUI(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Near-black cabinet background with a CRT scanline texture, inside a glowing
        # magenta chassis border.
        scanline_tex = make_scanline_texture(BG_SCANLINE_1, BG_SCANLINE_2)
        with self.canvas.before:
            Color(1, 1, 1, 1)
            self._bg_rect = Rectangle(pos=self.pos, size=self.size, texture=scanline_tex)
            Color(*ACCENT)
    ...
    def _update_bg(self, instance, value):
        ...
        tile_screen_px = 4  # tight scanlines, a few px each
